...
the stable domain identifier exchanged between all components — backend,
agents, and mis-clients. A UUID does not reveal database structure or user
volume and requires no separate external identifier.
...
as a second factor, ending with First Client Bootstrap.
